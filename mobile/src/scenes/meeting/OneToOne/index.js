...
export default function OneToOneMeetingViewer() {
  ...
  return (
    <>
      ...
      {/* Stream Preview - Desktop capture on top, mobile camera below */}
      <View style={{ flex: 1, marginTop: 8, marginBottom: 12 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <Text style={{ fontSize: 18, fontFamily: ROBOTO_FONTS.RobotoBold, color: colors.primary[100] }}>Stream Preview</Text>
          <Text style={{ fontSize: 12, color: colors.primary[300] }}>Live</Text>
        </View>

        {/* Desktop capture - occupies top 65% */}
        <View style={{ height: '65%', backgroundColor: '#000', borderRadius: 12, overflow: 'hidden', marginBottom: 8 }}>
          {localScreenShareOn ? (
            <LocalParticipantPresenter />
          ) : participantCount > 1 ? (
            <LargeView participantId={participantIds[1]} openStatsBottomSheet={openStatsBottomSheet} />
          ) : (
            <View style={{ flex:1, justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ color: colors.primary[300] }}>Waiting for desktop capture...</Text>
            </View>
          )}
        </View>

        {/* Mobile camera - occupies bottom 35% */}
        <View style={{ height: '30%', backgroundColor: '#0D0D0D', borderRadius: 12, overflow: 'hidden' }}>
          {participantCount > 1 ? (
            <MiniView participantId={participantIds[0]} openStatsBottomSheet={openStatsBottomSheet} />
          ) : (
      ...
      >
        {audioDevice.map((device, index) => {
          return (
            <React.Fragment key={device + '_' + index}>
              <MenuItem
                title={
